...
const defaultDialogAttr = {
  ...
  inputValue: "", // 输入框对象
  placeholder: "输入您的手机号", // 输入框placeholder
  maxLength: 11, // 输入框长度

  loginBtn: "获取验证码", // 登录按钮获取验证码按钮文本

  sending: true, // 是否可以发送验证码
  second: -1, // 倒计时间 second > 0:显示倒计时 second = 0:重新发送  second = -1:生么都不做
  clearSmsTime: null, // 倒计时间定时任务引用，关闭登录层清楚定时任务
};
...
export default {
  ...
  mounted() {
    ...
    // this event is called from @/weixin/callback
    // 微信登录回调处理
    let self = this;
  ...
  methods: {
    ...
    weixinLogin() {
      ...
      // initialize weixin parameters
      weixinApi.getLoginParam().then((response) => {
        // to display QR code
        var obj = new WxLogin({
          self_redirect: true,
          id: "weixinLogin", // 需要显示的容器id, in HTML
